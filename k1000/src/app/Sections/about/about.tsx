    ...
    <div className="w-full flex flex-col items-center space-y-32 py-10 font-sans bg-black overflow-x-hidden">
      ...
      <section className="relative w-full h-[55vh] rounded-[40px] overflow-hidden border border-cyan-500/20 shadow-2xl">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-10">
          ...
          <p className="text-white/50 mt-4 font-[Orbitron] tracking-[0.5em] text-xs uppercase">
      ...
      <section className="w-full max-w-6xl grid md:grid-cols-2 gap-16 items-center px-6">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <span className={`${conthrax} text-cyan-400 block text-lg tracking-widest uppercase`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 justify-items-center">
            {board.map((m) => (
              ...
              >
                ...
                <motion.h3 layoutId={`name-${m.id}`} className={`${conthrax} text-lg text-white mt-6 text-center leading-tight`}>
                  {m.name}
                </motion.h3>
                <motion.p layoutId={`pos-${m.id}`} className="text-cyan-500/60 text-[9px] font-[Orbitron] uppercase mt-2 tracking-[0.3em]">
                  {m.position}
                </motion.p>
              </motion.div>
            ))}
                ...
                <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 pointer-events-none">
                  ...
                  >
                    ...
                    <div className="flex-1 p-10 md:p-16 flex flex-col justify-center space-y-8 relative">
                      ...
                      <motion.p className="text-white/60 text-base md:text-lg leading-relaxed font-light text-justify">
              ...
              <div className={`grid grid-cols-1 ${gridClass} gap-x-20 gap-y-28 w-full justify-items-center`}>
                {grp.members.map((m, mi) => (
                  <motion.div key={mi} className="flex flex-col items-center text-center group">
                    ...
                    <p className="text-cyan-400 text-[12px] font-[Orbitron] tracking-[0.3em] mt-2 uppercase font-bold">
                      {m.position}
                    </p>

                    {/* ✅ ONLY ADDITION: BRANCH */}
                    {m.branch && (
                      <p className="text-white/40 text-[11px] font-[Orbitron] tracking-[0.25em] mt-1 uppercase">
                        {m.branch}
                      </p>
                    )}
                  </motion.div>
